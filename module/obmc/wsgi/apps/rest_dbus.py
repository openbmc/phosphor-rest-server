...
class DownloadDumpUtils:
    ...
    dump_loc = '/var/log/dumps/'
    file_suffix = 'tar'

    @classmethod
    def do_download(cls, dumpid):
        dump_loc = os.path.join(cls.dump_loc, dumpid)
        if not os.path.exists(dump_loc):
            abort(404, "File not found")

        files = glob.glob(dump_loc + '/*.' + cls.file_suffix)
        num_files = len(files)
        ...
        return static_file(dump_file, root=dump_loc, download=True)
...
class DownloadDumpHandler(RouteHandler):
    ...
    def do_get(self, dumpid):
        ...
        return DownloadDumpUtils.do_download(dumpid)
...
class JsonApiResponsePlugin(object):
    ...
    def apply(self, callback, route):
        content_type = getattr(
            route.get_undecorated_callback(), '_content_type', None)
        if (self.json_type != content_type and "" != content_type):
            return callback
